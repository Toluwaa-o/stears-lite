...
const CountriesHeader = ({ sortFieldChange, setAscending, sortField, ascending }: { sortFieldChange: (field: string) => void, setAscending: (field: boolean) => void, sortField: string, ascending: boolean }) => {
            ...
            if (indicatorCategories[key].indexOf(title) >= 0) {
                return (
                    <th
                        ...
                        className={`px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer ${category_colors[key]} max-w-[12rem] h-[3rem]`}
                    >
    ...
    return (
        <thead className="bg-gray-50">
            <tr>
                <th className={`px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-normal break-words max-w-[12rem] cursor-pointer`} onClick={() => clickHandler('name')}>
